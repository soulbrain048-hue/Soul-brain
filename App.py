...
class WiresharkModule:
    ...
    def __init__(self, executor: CommandExecutor, config: Config, logger: Logger):
    ...
    def check_tshark(self):
        success, _ = self.executor.run("which tshark", timeout=5)
        if not success:
            self.logger.warning("tshark not found! Install: sudo apt install tshark")
...
class KaliToolsSystem:
    ...
    def __init__(self):
        ...
        self.wireshark = WiresharkModule(self.executor, self.config, self.logger)
        self.scanner = NetworkScanner(self.executor, self.logger)
        self.web_tester = WebAppTester(self.executor, self.logger)
        self.wireless = WirelessTools(self.executor, self.logger)
        self.forensics = ForensicsTools(self.executor, self.logger)
        self.exploitation = ExploitationFramework(self.executor, self.logger)
        self.system = SystemUtils(self.executor, self.logger)
        
        # NEW MODULES
        self.osint = OSINTModule(self.executor, self.config, self.logger)
        self.malware = MalwareAnalysis(self.executor, self.config, self.logger)
        self.cloud = CloudSecurity(self.executor, self.logger)
        self.container = ContainerSecurity(self.executor, self.logger)
        self.api_sec = APISecurity(self.executor, self.logger)
        self.crypto = CryptoTools(self.executor, self.logger)
        self.bugbounty = BugBountyTools(self.executor, self.logger)
